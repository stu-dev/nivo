...
on, for example if you want to make some change on the `@stu-dev/nivo-bar` package,
...
You can also build the packages without running a watcher, you have two options:
 ...
 2. Rebuild only a specific package, for example `make pkg-build-bar` for `@stu-dev/nivo-bar` package
